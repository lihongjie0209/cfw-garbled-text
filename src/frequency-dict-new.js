...
try {
  ...
  console.log(`加载了 ${Object.keys(chineseFrequencyFromCSV).length} 个中文字符频率数据`);
} catch (error) {
  console.warn('无法加载CSV转换的频率数据，使用默认数据:', error.message);
...
console.log(`频率字典统计:
- 中文字符: ${Object.keys(chineseFrequency).length} 个
- 英文字符: ${Object.keys(englishFrequency).length} 个  
- 数字字符: ${Object.keys(numberFrequency).length} 个
- 总计: ${Object.keys(combinedFrequency).length} 个`);
